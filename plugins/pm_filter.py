...
async def next_page(bot, query):
    ...
    btn.insert(0, 
        [
            InlineKeyboardButton(text="⚡ʜᴏᴡ ᴛᴏ ᴠᴇʀɪғʏ⚡", url=HOW_TO_VERIFY)
        ]
    )
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('ɢʀᴏᴜᴘ', url='https://t.me/+P-zPGy3Gjy1kOWQ9')
        ], [
...
async def auto_filter(client, msg, spoll=False):
    ...
    cap = f"<b>𝖧𝖾𝗋𝖾 𝖠𝗋𝖾 𝖳𝗁𝖾 𝖱𝖾𝗌𝗎𝗅𝗍𝗌 𝖥𝗈𝗋 𝖸𝗈𝗎𝗋 𝖰𝗎𝖾𝗋𝗒 <code>{search}</code></b>"
    dlt = await message.reply_text(cap, reply_markup=InlineKeyboardMarkup(btn))
